...
TERMUX_PKG_SRCURL=https://fossies.org/linux/misc/-${TERMUX_PKG_VERSION}.tar.xz
...
termux_step_configure() {
	...
	#  does not use configure. But place man pages at correct path:
	sed -i "s@(PREFIX)/man@(PREFIX)/share/man@g" $TERMUX_PKG_SRCDIR/Makefile
}

termux_step_make() {
	#  uses a separate makefile for the shared library
...
termux_step_make_install() {
	...
	(cd $TERMUX_PREFIX/bin && ln -s  bzcat)
	(cd $TERMUX_PREFIX/bin && ln -s  bunzip2)
